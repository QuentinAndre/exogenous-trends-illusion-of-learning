...
i_am("Studies/Study2/Code/S2_DataAnalysis_MLogit.R")
source(here("custom_theme.R"))

# Load the data
choices_data <- read_csv(here("Studies/Study2", "Data", "game_choices_clean_with_attributes.csv")) %>%
...
levels <- c(
  "Customers", "Competitors", "Valuation", "Electronics vs. Biotech", "Robotics vs. Biotech"
)
...
df_contrasts <- data.frame(
  ...
  slope = factor(slope, levels = c(1, 0), labels = c(
    "Increasing",
    "Flat"
  ))
)
